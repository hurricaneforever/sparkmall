...
object CategorySessionTop10App {
  def main(args: Array[String]): Unit = {
    ...
    //转换数据结构  （ categoryid+sessionid, 1L ）
    val makeRdd: RDD[(String, Long)] = filterRDD.map(log => {
      (log.click_category_id + "_" + log.session_id, 1L)
    })
    //聚合
    val reduceRDD: RDD[(String, Long)] = makeRdd.reduceByKey(_ + _)
    //转换数据结构 (categoryid，(sessionid, sum))
